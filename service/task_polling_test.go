...
func (a *transientPollingAdaptor) FetchTask(_ string, _ string, _ *model.Task, _ string) (*http.Response, error) {
	a.calls++
	if a.calls == 1 {
...
func (a *transientPollingAdaptor) ParseTaskResult(*model.Task, *http.Response, []byte) (*relaycommon.TaskInfo, error) {
...
func TestFetchTaskWithRetryRetriesTransientResponses(t *testing.T) {
	...
	defer cancel()
	resp, err := fetchTaskWithRetry(ctx, adaptor, "https://provider.example", "KEY", &model.Task{TaskID: "task"}, "")
