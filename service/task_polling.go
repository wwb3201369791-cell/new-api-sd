...
func updateVideoSingleTask(ctx context.Context, adaptor TaskPollingAdaptor, ch *model.Channel, taskId string, taskM map[string]*model.Task) error {
	...
	snap := task.Snapshot()
	resp, err := fetchTaskWithRetry(ctx, adaptor, baseURL, key, task, proxy)
	if err != nil {
...
func fetchTaskWithRetry(ctx context.Context, adaptor TaskPollingAdaptor, baseURL, key string, task *model.Task, proxy string) (*http.Response, error) {
	const maxAttempts = 3
	var lastErr error
