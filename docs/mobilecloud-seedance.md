...
管理员和 API 用户的分步操作、PowerShell/curl 示例及验收判定见
...
按上游 completion token 结算的阶梯表达式：移动云上游价位中无输入视频时
480p/720p 为 46 元/百万 token、1080p 为 51 元/百万 token、4k 为 26 元/百万
token；有输入视频时分别为 28、31 和 16 元/百万 token。网关内置默认价按当前
平台倍率折算为 92、102、52 和 56、62、32 元/百万 token。管理员仍可在分组与
模型定价设置中覆盖这组默认值，覆盖后以数据库中的表达式为准。4k 是否能实际生成还取决于
移动云渠道和上游模型是否开通该能力；网关会透传 `resolution: "4k"` 并按上游
最终返回的用量结算。

请求未指定或无法识别 `resolution` 时，提交阶段按 1080p 保守预估预扣，完成后
使用上游实际 token/分辨率结算并退补差额；建议客户端始终显式传入分辨率，避免
预估金额暂时高于最终金额。
...
客户端响应使用统一的网关字段；渠道名称、厂商专有响应头和原始审核码不会返回给
API 用户，仅在管理员可见的脱敏日志中保留。素材详情中的 `assetUrl` 是后续视频任务
所需的媒体地址，客户端只在提交任务或预览期间使用，不应作为长期公开链接保存。
