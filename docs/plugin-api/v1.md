...
`sharedModels: true` is for task-provider plugins that intentionally expose the same public model name (for example, two Seedance upstreams). The host keeps all matching protocol candidates and filters channels by each channel's `task_plugin_key`, so administrators can load-balance providers without changing the client-facing model name. Leave it unset for ordinary plugins; overlapping claims remain a validation error.
...
Plugins that can validate an upstream without creating a billable task may
optionally export `buildChannelTestRequest(ctx)`. The administrator channel
test invokes this hook instead of `buildSubmitRequest`, validates the returned
URL against the configured channel host, and performs the request with the
channel credential. A successful 2xx response is considered a healthy
connection; no task, billing record, or quota settlement is created. The hook
receives `{model, upstreamModel, baseUrl, apiKey, action: "channel_test"}` and
may return the same request descriptor shape. Plugins should use a provider's
non-billing health, model-discovery, or credential-validation endpoint here. If
the provider only exposes a resource lookup, a sentinel request may declare
`acceptedStatusCodes` (for example `[404]`) and `acceptErrorResponse: true` so
the expected not-found envelope still validates the credential without billing.
...
| `openai_video`     | list      | `GET /v1/videos`         | none                              |
| `openai_video`     | delete    | `DELETE /v1/videos/:task_id` | none                         |
| `openai_video`     | create    | `POST /api/v3/contents/generations/tasks` | JSON |
| `openai_video`     | retrieve  | `GET /api/v3/contents/generations/tasks/:task_id` | none |
| `openai_video`     | list      | `GET /api/v3/contents/generations/tasks` | none |
| `openai_video`     | delete    | `DELETE /api/v3/contents/generations/tasks/:task_id` | none |
...
Plugins that expose provider-side cancellation or deletion may optionally export `buildControlRequest(ctx)`. The host supplies the public and upstream task IDs, the requested `cancel`/`delete` action, the selected channel base URL and authentication context. Only `DELETE` or `POST` requests are accepted and the URL is validated against the channel's allowed hosts.
