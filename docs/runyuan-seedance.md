...
Runyuan 的素材接口目前已按移动云适配层的能力补齐：素材组和素材的创建、列表、详情、更新、删除，以及真人认证会话和结果查询，均通过同一套网关路径转发。查询类 Action 在连接提前关闭时会重新生成签名并做有限重试；创建、更新、删除和认证会话不会自动重放，避免重复资源或重复认证。

Runyuan 文档约定同一账号的 AIGC 素材组可能是账号级共享资源。网关仍为每个 New API 客户建立独立的本地默认组索引；当上游组 ID 被多个客户复用时，素材列表还会按本地素材索引二次过滤，防止一个客户读取到另一个客户通过网关登记的素材。真实的 Runyuan AK/SK 到位前，可以使用本地 HTTP 测试桩验证签名、字段和状态映射；真实素材下载、审核和计费仍需厂商测试账号验收。

因此，Runyuan 渠道的“创建素材组”仍保留在统一 API 中，但当上游账号级 AIGC 组已存在时会返回 `409 ASSET_GROUP_PROVIDER_LIMIT`，引导客户使用自己的默认组；如果历史数据中同一上游组已被多个客户索引，更新和删除会返回 `409 ASSET_GROUP_SHARED`，不会误改或删除其他客户正在使用的上游组。移动云渠道不受此限制，可以按客户创建多个上游素材组。
